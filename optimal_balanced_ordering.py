...
for i in range(len(set(df_all['供应商']))):
    ...
    if len(df) > 1:
        ...
        if sum(T - T_ori >= 1) > 0:
            raise Exception('原始数据中周转天数向下取整有误，请检查')
        critic = T.max()
        ...
        stock, arri, order, dms = np.array(df['当前库存']), np.array(df['在途库存']), \
            np.array(df['计算订货量']), np.array(df['日均销量'])
        ...
        exact_unit = exact_order / unit
        ceil_unit, floor_unit = np.ceil(exact_unit), np.floor(exact_unit)
        if (ceil_unit - exact_unit) / ceil_unit <= 0.01:
